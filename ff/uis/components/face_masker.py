...
import ff.choices
from ff import face_masker, state_manager, wording
from ff.common_helper import calc_float_step, calc_int_step
from ff.types import FaceMaskRegion, FaceMaskType, FaceOccluderModel, FaceParserModel
...
def render() -> None:
	...
	FACE_MASK_BLUR_SLIDER = gradio.Slider(
		...
		step = calc_float_step(ff.choices.face_mask_blur_range),
	...
	with gradio.Group():
		with gradio.Row():
			FACE_MASK_PADDING_TOP_SLIDER = gradio.Slider(
				...
				step = calc_int_step(ff.choices.face_mask_padding_range),
